...
import (
	"fmt"
	calculatorErr "project/internal/errors"
	"strconv"
	"strings"
)
...
func tokenizeExpression(expression string) ([]string, error) {
		...
		case '+', '-', '*', '/', '(', ')':
			...
			if char == ')' {
				...
				if n == 0 { // Неправильный порядок скобок " ) ("
					return nil, calculatorErr.ErrMissingBracket
				}
	...
	if len(tokens) == 0 {
		return nil, calculatorErr.ErrEmptyExpression
	}
...
func evaluateTokens(tokens []string) (float64, error) {
	...
	for _, token := range tokens {
		...
		case ")":
			...
			if len(operators) == 0 {
				return 0, calculatorErr.ErrMissingBracket
			}
	...
	if len(stack) != 1 {
		return 0, calculatorErr.ErrInvalidExpression
	}
...
func applyOperator(stack *[]float64, operator string, fn func(float64, float64) float64) error {
	if len(*stack) < 2 {
		return calculatorErr.ErrNotEnogthOperand
	}
	...
	*stack = (*stack)[:len(*stack)-1]
	...
	if operator == "/" && b == 0 {
		return calculatorErr.ErrDivisionByZero
	}
...
func TtokenizeExpression(expression string) ([]string, error) {
